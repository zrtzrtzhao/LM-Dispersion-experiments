...
def evaluate_checkpoint(
    ...
    with tempfile.TemporaryDirectory() as tmp_cache:
        resolved_cache_dir = cache_dir or tmp_cache
        tokenizer_source = tokenizer_name or ckpt_path
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_source, cache_dir=resolved_cache_dir)
        config = AutoConfig.from_pretrained(ckpt_path, cache_dir=resolved_cache_dir)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare Qwen3 layer-wise spectral metrics for angular_spread and SFA-Peak checkpoints."
    )
    ...
    parser.add_argument("--output", default="./figures/qwen3_spectral_metrics_sfa_peak_vs_angular.png")
    parser.add_argument("--csv_output", default="./figures/qwen3_spectral_metrics_sfa_peak_vs_angular.csv")
    parser.add_argument(
        "--tokenizer_name",
        default=None,
        help="Optional tokenizer source. Use this when checkpoints do not contain tokenizer files.",
    )
    parser.add_argument(
        "--cache_dir",
        default=None,
        help="Optional Hugging Face cache directory for tokenizer/model config loading.",
    )
