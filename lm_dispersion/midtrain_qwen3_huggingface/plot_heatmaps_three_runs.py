...
def compute_histories_for_checkpoint(tokenizer, model, device, repetitions, max_length):
    ...
    for random_seed in range(repetitions):
        ...
        if cossim_matrix_by_layer is None:
            cossim_matrix_by_layer = [[m] for m in curr]
        else:
            for i in range(len(cossim_matrix_by_layer)):
                cossim_matrix_by_layer[i].append(curr[i])

    for i in range(len(cossim_matrix_by_layer)):
        min_tokens = min(m.shape[0] for m in cossim_matrix_by_layer[i])
        cropped = [m[:min_tokens, :min_tokens] for m in cossim_matrix_by_layer[i]]
        cossim_matrix_by_layer[i] = np.stack(cropped, axis=0).mean(axis=0)
    ...
    for row_idx, (label, _) in enumerate(tqdm(runs_in_fig, desc="runs")):
        ...
        for col_idx in tqdm(range(max_ckpts), desc=label, leave=False):
            ...
            with tempfile.TemporaryDirectory() as tmp_cache:
                cache_dir = args.cache_dir or tmp_cache
                tokenizer_source = args.tokenizer_name or ckpt_path
                tokenizer = AutoTokenizer.from_pretrained(tokenizer_source, cache_dir=cache_dir)
                try:
                    config = AutoConfig.from_pretrained(ckpt_path, cache_dir=cache_dir)
