...
def write_summary_csv(rows, output_dir):
    path = output_dir / "main_results_summary.csv"
    fields = [
        "Model",
        "Run",
        "10-task avg",
...
def write_task_csv(rows, output_dir):
    path = output_dir / "main_results_10task_detail.csv"
...
def write_markdown(rows, output_dir):
    path = output_dir / "main_results_summary.md"
...
def write_latex(rows, output_dir):
    path = output_dir / "main_results_summary.tex"
    with path.open("w", encoding="utf-8") as f:
...
def write_best_by_model_csv(rows, output_dir):
    path = output_dir / "main_results_best_by_model.csv"
    fields = [
        "Model",
        "Best run",
        "10-task avg",
...
def write_delta_csv(rows, output_dir):
    path = output_dir / "main_results_delta_vs_ce.csv"
    fields = [
        "Model",
        "Run",
        "10-task avg",
...
def write_delta_latex(rows, output_dir):
    ...
    path = tables_dir / "main_results_delta_vs_ce.tex"
    baselines = baseline_by_model(rows)
    with path.open("w", encoding="utf-8") as f:
...
def write_figures(rows, output_dir):
    ...
    for model in sorted({row["model"] for row in rows}):
        ...
        fig.tight_layout()
        path = figure_dir / f"main_results_10task_avg_{safe_name(model)}.png"
...
def main():
    parser = argparse.ArgumentParser(description="Summarize server-side GPT-2 midtraining results.")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional exact model filter, e.g. gpt2 or gpt2-medium.",
    )
