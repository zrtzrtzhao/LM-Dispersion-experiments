...
class CustomLossTrainer(Trainer):
    ...
    def _apply_sfa_to_final_hidden(
        self,
        hidden: torch.Tensor,
        labels: torch.Tensor,
        ...
        final hidden states, then suppress that direction in the raw hidden states.
        The auxiliary CE is computed on the full SFA-perturbed final hidden states.
        """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if want_sfa_ce:
            ...
            z_sfa = self._apply_sfa_to_final_hidden(
                final_hidden,
                labels=labels,
                attention_mask=inputs.get("attention_mask"),
            )
            sfa_logits = self._lm_head_logits(model, z_sfa)
            sfa_ce_loss = self.loss_fn(sfa_logits, labels)
...
def main(args):
    ...
    log(
        ...
        f"k={args.sfa_iterations} | strength={args.sfa_strength} | "
