"""
Mid-train GPT-2 with SFA-Peak spectral suppression.

This script keeps the normal causal-LM forward path unchanged. During training
only, it adds a direct spectral loss that penalizes the dominant hidden-state
direction energy ratio. Inference and lm-eval use the unmodified model.
"""
...
class CustomLossTrainer(Trainer):
    ...
    def _sfa_peak_loss_one_layer(
        ...
        ||H r||_2^2 / ||H||_F^2, where r estimates the dominant right singular
        direction of row-normalized valid prediction hidden states.
        """
        if hidden.size(-1) < 2:
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Mid-train GPT-2 with SFA-Peak spectral suppression loss.")
    ...
    ap.add_argument("--sfa_peak_iterations", type=int, default=2,
                    help="Power-iteration steps for estimating the dominant spectral direction.")
    ...
    ap.add_argument("--sfa_normalize_rows", action="store_true", default=True,
                    help="Row-normalize hidden states before computing the spectral energy ratio.")
    ap.add_argument("--no_sfa_normalize_rows", action="store_false", dest="sfa_normalize_rows",
                    help="Compute SFA-Peak on raw hidden states instead of row-normalized directions.")
    ...
    if args.sfa_peak and not args.sfa_normalize_rows:
        sfa_tag += "-raw"
