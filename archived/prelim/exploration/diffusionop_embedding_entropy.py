...
from diffusion.diffusion_operator import apply_diffusion_operator
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        z_0 = torch.nn.functional.normalize(z_0, dim=1, p=2).cpu()

        embeddings_by_layer = apply_diffusion_operator(X=z_0, t=40)
        embeddings_by_layer = [z_0] + embeddings_by_layer
        embeddings_by_layer = [item.numpy() for item in embeddings_by_layer]
    ...
    plot_similarity_histograms(
        cossim_matrix_by_layer,
        step=1,
        save_path='../../visualization/diffusion_op/embedding_cossim_histogram_albert_xlarge_v2.png')

    # Plot and save metrics (prob density, entropy, etc.).
    plot_probability(
        cossim_matrix_by_layer,
        save_path='../../visualization/diffusion_op/embedding_cossim_probability_albert_xlarge_v2.png')
    plot_entropy(
        cossim_matrix_by_layer,
        save_path='../../visualization/diffusion_op/embedding_cossim_entropy_albert_xlarge_v2.png')
    plot_DSE(
        embeddings_by_layer,
        save_path='../../visualization/diffusion_op/embedding_DSE_albert_xlarge_v2.png')
